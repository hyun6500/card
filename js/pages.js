  ...
  out.querySelector("#an-add").onclick = async () => {
    ...
    try {
      ...
      const fields = packEventFields({
        ...
        data: { cond, benefits: bens, cautions: r.cautions || [], shots: [], summary: r.summary || "" },
        raw: keep.slice(0, 20000), memo: ""
      });
      await api("add", { table: "이벤트", fields, images: imgs || [] });
      closeModal();
      toast("후보군에 담았어요.");
      await loadAll();
      S.tab = "promo"; render();
      ...
      btn.disabled = false; btn.textContent = "후보군에 담기";
    }
  };
