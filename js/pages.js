...
/* 화면 안에서 목록을 갈아 끼우는 작은 탭 */
function segTab(items, current, onPick) {
  const box = el(`<section class="seg">
    ${items.map(i => `<button class="seg-btn${i.key === current ? " on" : ""}" data-k="${esc(i.key)}">
      ${esc(i.label)}${i.count != null ? `<em>${i.count}</em>` : ""}
    </button>`).join("")}
  </section>`);
  box.querySelectorAll(".seg-btn").forEach(b => { b.onclick = () => onPick(b.dataset.k); });
  return box;
}
...
function render() {
  ...
  else if (S.tab === "stats") renderStats(main);
  ...
  if (fab) fab.classList.toggle("hide", S.tab !== "promo");
  window.scrollTo(0, 0);
}
...
function renderCards(main) {
  ...
  const active = sortByRecent(mine.filter(c => ["t1", "t2"].indexOf(usageTier(c).key) >= 0));
  const resting = sortByRecent(mine.filter(c => ["t1", "t2"].indexOf(usageTier(c).key) < 0));

  /* 히어로: 누르면 결산 탭으로 */
  const hero = el(`
    <section class="hero tapcard" tabindex="0" role="button">
      <div class="hero-label">도전 중인 혜택으로 들어올 돈</div>
      <div class="hero-num">${fWonFull(sum.expect)}<span class="won">원</span></div>
      <div class="hero-sub">
        <span class="hlink" data-go="report">받은 캐시백 ${fWonFull(sum.got)}원</span>
        <span class="hlink" data-go="promo">참여 중 ${S.events.filter(e => e.status === "신청").length}건</span>
        ${sum.maybe ? `<span class="hlink" data-go="promo">고민 중 ${fWon(sum.maybe)}</span>` : ""}
        ${sum.todo ? `<span class="hlink" data-go="promo">확인할 것 ${sum.todo}건</span>` : ""}
      </div>
    </section>`);
  hero.onclick = () => goTab("report");
  hero.onkeydown = e => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); goTab("report"); } };
  hero.querySelectorAll(".hlink").forEach(a => {
    a.onclick = e => { e.stopPropagation(); goTab(a.dataset.go); };
  });
  main.appendChild(hero);

  /* 지금 응모할 수 있는 건이 있으면 하나만 알린다 */
  if (hasMyData() && !snoozedUntil()) {
    const ch = openChances()[0];
    if (ch) {
      const note = el(`
        <section class="chance">
          <div class="chance-body tapcard" tabindex="0" role="button">
            <div class="chance-tag">지금 응모할 수 있어요${ch.kind === "popular" ? " | " + ch.count + "명이 담아둔 프로모션" : ""}</div>
            <b>${esc(ch.issuer)} ${esc(ch.name)}</b>
            <div class="chance-sub">${ch.total ? "최대 " + fWon(ch.total) : ""}${ch.dd != null ? (ch.total ? " | " : "") + "응모 마감 " + fDateK(ch.end) + " (D-" + ch.dd + ")" : ""}</div>
          </div>
          <button class="chance-x" aria-label="닫기">X</button>
        </section>`);
      note.querySelector(".chance-body").onclick = () => {
        if (ch.kind === "mine") openEventDetail(ch.ev.no);
        else openPromoStat(ch.pr);
      };
      note.querySelector(".chance-x").onclick = e => {
        e.stopPropagation();
        const m = openModal(`
          <h3>이 알림을 어떻게 할까요?</h3>
          <p class="hint">지금 응모할 수 있는 캐시백이 생기면 알려드리는 자리입니다.</p>
          <div class="modal-btns wrapbtns">
            <button class="cta" id="sn-week">일주일간 안 보기</button>
            <button class="mini ghost" id="sn-close">그냥 닫기</button>
          </div>`);
        m.querySelector("#sn-week").onclick = () => { snoozeWeek(); closeModal(); render(); };
        m.querySelector("#sn-close").onclick = () => { closeModal(); note.remove(); };
      };
  ...
  const view = S.cardTab || "active";
  main.appendChild(segTab([
    { key: "active", label: "쓰는 중", count: active.length },
    { key: "resting", label: "쉬는 중", count: resting.length },
    { key: "all", label: "전체", count: mine.length }
  ], view, k => { S.cardTab = k; render(); }));

  const shown = view === "active" ? active : view === "resting" ? resting : sortByRecent(mine);
  if (!shown.length) {
    main.appendChild(el(`<div class="empty">${view === "active" ? "6개월 안에 쓴 카드가 없어요."
      : view === "resting" ? "6개월 넘게 쉰 카드가 없어요." : "등록한 카드가 없어요."}<br>+ 카드 추가를 누르거나, 프로모션을 신청하면 카드가 자동으로 등록됩니다.</div>`));
  } else {
    main.appendChild(cardGrid(shown));
    if (view === "resting") {
      main.appendChild(el(`<p class="hint" style="text-align:center">6개월 넘게 쉰 카드는 무실적 조건에 유리합니다.</p>`));
    }
  }

  /* 카드사별 응모 가능, 가능일이 가까운 순 */
  main.appendChild(el(`<div class="sec-head"><h2>카드사별 응모 가능</h2><span class="sec-sub">담아둔 공고 조건 반영</span></div>`));
  const list = el(`<section class="issuer-list"></section>`);
  CONFIG.ISSUERS.map(iss => {
    const info = issuerCondInfo(iss);
    const j = judge(iss, info.cond);
    return { iss, info, j, sort: j.hardBlock ? Infinity : j.openDay.getTime() };
  }).sort((a, b) => a.sort - b.sort).forEach(({ iss, info, j }) => {
    const checks = conditionChecks(iss, info.cond);
    const cls = j.hardBlock ? "no" : j.ok ? "ok" : "wait";
    const txt = j.hardBlock ? "대상 제외" : j.ok ? "지금 가능" : "D-" + j.wait;
    list.appendChild(el(`
      <div class="ic ${cls}">
        <div class="ic-head">
          <b>${esc(iss)}</b>
          ${info.from ? `<span class="ic-src">${esc(info.from)} 조건</span>` : '<span class="ic-src def">기본 조건</span>'}
          <span class="pill ${cls}">${txt}</span>
        </div>
        ${issuerTel(iss) ? `<a class="teltag" href="tel:${esc(issuerTel(iss).replace(/-/g, ""))}" onclick="event.stopPropagation()">${esc(issuerTel(iss))} 전화</a>` : ""}
        ${checks.map(c => `
          <div class="ck ${c.ok ? "good" : "bad"}">
            <i>${c.ok ? "충족" : "미충족"}</i>
            <div class="ck-body">
              <div class="ck-rule"><b>${esc(c.name)}</b> ${esc(c.rule)}${c.def ? ' <em class="ck-def">기본값</em>' : ""}</div>
              <div class="ck-mine">내 상태 ${esc(c.mine)}${c.gap ? ", " + esc(c.gap) : ""}</div>
            </div>
          </div>`).join("")}
      </div>`));
  });
...
/* 세로형 카드 격자. 정보는 카드 면 안에 얹는다. */
function cardGrid(cards) {
  const g = el(`<section class="cgrid"></section>`);
  cards.forEach(c => {
    ...
    const j = judge(c.issuer, issuerCondInfo(c.issuer).cond);

    const box = el(`
      <div class="vc tapcard${img ? " has-img" : ""}" style="--c1:${col[0]};--c2:${col[1]}" tabindex="0" role="button">
        ${img ? `<img class="vc-img" src="${esc(img)}" alt="" referrerpolicy="no-referrer" onerror="this.closest('.vc').classList.remove('has-img');this.remove()">` : ""}
        <div class="vc-top">
          <span class="vc-issuer">${esc(c.issuer)}</span>
          ${evs.length ? '<span class="vc-flag">참여 중</span>'
        : j.hardBlock ? '<span class="vc-flag off">제외</span>'
          : j.ok ? '<span class="vc-flag ok">응모 가능</span>'
            : `<span class="vc-flag wait">D-${j.wait}</span>`}
        </div>
        <div class="vc-mid">
          <span class="vc-chip"></span>
          ${c.brand ? `<span class="vc-brand">${esc(c.brand)}</span>` : ""}
        </div>
        <div class="vc-bot">
          <b class="vc-name">${esc(c.name || "카드명 미입력")}</b>
          <div class="vc-line">
            <i class="dot ${t.key}"></i>
            <span>${agoText(c)}</span>
            <button class="vc-btn" title="오늘 사용한 것으로 기록">오늘</button>
          </div>
          ${qsMax ? `<div class="vc-warn">${fDateK(qsMax)}까지 해지 금지</div>` : ""}
          ${c.fee ? `<div class="vc-sub">연회비 ${fWon(c.fee)}</div>` : ""}
        </div>
      </div>`);

    box.onclick = () => openCardForm(c);
    box.onkeydown = e => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); openCardForm(c); } };
    box.querySelector(".vc-btn").onclick = async e => {
      e.stopPropagation();
      try {
        await api("update", { table: "카드", no: c.no, fields: { "최근사용일": fDate(today()) } });
        toast(c.issuer + " 사용일을 오늘로 기록했어요.");
...
function openCardForm(c) {
  ...
  const m = openModal(`
    ...
    <div id="cf-tel">${issuerTel(c.issuer) ? `<a class="teltag" href="tel:${esc(issuerTel(c.issuer).replace(/-/g, ""))}">${esc(c.issuer)}카드 고객센터 ${esc(issuerTel(c.issuer))}</a>` : ""}</div>
    <div class="modal-btns">
      ${isNew ? "" : '<button class="mini ghost danger" id="cf-del">삭제</button>'}
      <button class="cta" id="cf-save">저장</button>
    </div>`);
  bindCardFinder(m);
  const issSel = m.querySelector("#cf-issuer");
  issSel.onchange = () => {
    const tel = issuerTel(issSel.value);
    m.querySelector("#cf-tel").innerHTML = tel
      ? `<a class="teltag" href="tel:${tel.replace(/-/g, "")}">${esc(issSel.value)}카드 고객센터 ${tel}</a>` : "";
  };
...
function renderPromo(main) {
  const active = S.events.filter(e => e.status === "신청");
  const cand = S.events.filter(e => e.status === "후보");
  const view = S.promoTab || "active";

  main.appendChild(segTab([
    { key: "active", label: "참여 중", count: active.length },
    { key: "cand", label: "후보군", count: cand.length }
  ], view, k => { S.promoTab = k; render(); }));

  const list = view === "active" ? active : cand;
  if (!list.length) {
    main.appendChild(el(`<div class="empty">${view === "active"
      ? "참여 중인 프로모션이 없어요.<br>후보군에서 신청 처리를 하면 여기로 올라옵니다."
      : "담아둔 후보가 없어요.<br>오른쪽 아래 + 버튼으로 공고를 분석해 담아 보세요."}</div>`));
    return;
  }
  list.forEach(ev => main.appendChild(promoCard(ev, view === "active")));
}
...
function promoCard(ev, isActive) {
  ...
  const gotCount = tries.filter(b => b.paidDate).length;
  const gotAmt = tries.reduce((s, b) => s + (b.paidDate ? Number(b.paidAmt || b.back || 0) : 0), 0);
  ...
  tries.forEach((b, i) => {
    ...
    benRows += `
      <div class="ben ${st.cls}${b.paidDate ? " got" : ""}">
        <div class="ben-top">
          <b>${b.paidDate ? "받음 " : ""}${esc(b.kind || "혜택" + (i + 1))}</b>
          <span class="pill ${st.cls}">${b.paidDate ? fDateK(b.paidDate) + " 입금" : st.txt}</span>
        </div>
        <div class="ben-sum">${esc(b.summary || "")}</div>
        ${b.paidDate
      ? `<div class="ben-nums"><span>입금 완료</span><b class="green">+${fWon(b.paidAmt || b.back)}</b></div>`
      : b.need ? `
          <div class="bar"><i style="width:${pct}%"></i></div>
          <div class="ben-nums">
            <span>${fWon(b.spent || 0)} / ${fWon(b.need)}${left ? ", " + fWon(left) + " 남음" : ""}</span>
  ...
  const c = el(`
    <section class="promo tapcard ${v.cls}" tabindex="0" role="button">
      ...
      <div class="promo-total">
        <span>도전 중 ${tries.length}건${gotCount ? ", 받음 " + gotCount + "건" : ""}${others.length ? " / 전체 " + bens.length + "건" : ""}</span>
        <b class="green">${gotAmt ? fWonFull(gotAmt) + "원 받음" : fWon(tryTotal)}${gotAmt && tryTotal > gotAmt ? ' <em class="sub-amt">남은 ' + fWon(tryTotal - gotAmt) + "</em>" : (!gotAmt && tryTotal !== total ? ' <em class="sub-amt">최대 ' + fWon(total) + "</em>" : "")}</b>
      </div>
...
function openEventDetail(no) {
  ...
  bens.forEach((b, i) => {
    ...
    benHtml += `
      <div class="ben pick-${pickKey(b)} ${st.cls}" data-i="${i}">
        ...
        ${pickKey(b) === "try" ? `<button class="gotbtn${b.paidDate ? " on" : ""}" data-got="${i}">
          ${b.paidDate ? "받음 " + fDateK(b.paidDate) + (b.paidAmt ? ", " + fWon(b.paidAmt) : "") + " (누르면 취소)" : "캐시백 받았어요"}
        </button>` : ""}
        <div class="ben-body">
          <div class="ben-state">
  ...
  const m = openModal(`
    ...
    <div class="callbox">
      ...
      ${issuerTel(ev.issuer) ? `
        <a class="callbtn" href="tel:${esc(issuerTel(ev.issuer).replace(/-/g, ""))}">
          <b>${esc(ev.issuer)}카드 ${esc(issuerTel(ev.issuer))}</b>
          <span>눌러서 전화 걸기</span>
        </a>
        <p class="hint">무실적과 수혜 이력은 카드사 전산으로만 정확히 알 수 있어요. 응모 전에 한 번 확인하시면 조건을 다 채우고도 못 받는 일을 막을 수 있습니다.</p>`
      : ""}
    </div>
    <div class="modal-btns wrapbtns">
  ...
  m.querySelectorAll(".ben[data-i]").forEach(div => {
    ...
    const got = div.querySelector("[data-got]");
    if (got) got.onclick = async () => {
      if (bens[i].paidDate) {
        if (!confirm("받음 표시를 취소할까요?")) return;
        bens[i].paidDate = ""; bens[i].paidAmt = null;
      } else {
        bens[i].paidDate = fDate(today());
        if (bens[i].paidAmt == null || bens[i].paidAmt === "") bens[i].paidAmt = Number(bens[i].back || 0) || null;
      }
      try {
        await quietSave();
        closeModal();
        openEventDetail(ev.no);
        toast(bens[i].paidDate ? "받음으로 표시했어요. 금액이 다르면 아래에서 고치세요." : "받음 표시를 취소했어요.");
      } catch (e) { toast(e.message, true); }
    };
...
function openAnalyze() {
  ...
  m.querySelector("#an-img").onchange = async e => {
    ...
    for (const f of files) {
      const b64 = await shrinkImage(f, 1000);
      imgs.push(b64);
...
function renderAnalysis(m, r, rawText, imgs) {
  ...
  // 딸린 추가 이벤트를 붙일 수 있는 프로모션들. 카드사를 알면 그 카드사 것만.
  const mergeTargets = S.events.filter(e =>
    e.owner === S.user.name && (e.status === "신청" || e.status === "후보")
    && (!r.issuer || e.issuer === r.issuer) && (!dup || e.no !== dup.no));
  out.innerHTML = `
    <div class="an-result">
      <div class="promo-head">
        <div>
      ...
      ${mergeTargets.length ? `
        <div class="merge">
          <b>${r.issuer ? "이 내용을 기존 프로모션에 합칠까요?" : "어떤 프로모션의 추가 혜택인가요?"}</b>
          <span>${r.issuer ? "본 공고에 딸린 추가 이벤트라면, 새로 담지 말고 원래 프로모션에 혜택으로 붙이는 편이 관리하기 좋습니다."
        : "공고에서 카드사를 찾지 못했어요. 참여 중이거나 담아둔 프로모션 중에서 골라 주세요."}</span>
          <div class="merge-list">
            ${mergeTargets.map(e => `<button class="merge-item" data-no="${esc(e.no)}">
              <b>${esc(e.issuer)} ${esc(e.name)}</b>
              <span>${esc(e.status)}${e.platform ? " | " + esc(e.platform) : ""} | 혜택 ${(e.data.benefits || []).length}건</span>
            </button>`).join("")}
          </div>
        </div>` : ""}
      <div class="modal-btns wrapbtns">
        ${dup ? `<button class="cta" id="an-over">기존 이벤트에 덮어쓰기</button>
             ...
             : `<button class="cta${r.issuer ? "" : " ghost-cta"}" id="an-add">${r.issuer ? "새 프로모션으로 담기" : "그래도 새로 담기"}</button>`}
      </div>
    </div>`;
  ...
  // 기존 프로모션에 혜택으로 합치기
  out.querySelectorAll(".merge-item").forEach(b => {
    b.onclick = async () => {
      const target = S.events.find(e => e.no === b.dataset.no);
      if (!target) return;
      if (!confirm(target.issuer + " " + target.name + "\n\n이 프로모션에 혜택 " + bens.length + "건을 추가할까요?")) return;
      b.disabled = true;
      const old = b.innerHTML;
      b.innerHTML = "<b>합치는 중...</b>";
      try {
        const next = Object.assign({}, target);
        next.data = Object.assign({}, target.data);
        next.data.benefits = (target.data.benefits || []).concat(bens.map(normBenefit));
        next.data.cautions = (target.data.cautions || []).concat(r.cautions || []).slice(0, 20);
        next.memo = (target.memo ? target.memo + " / " : "") + (r.name || "추가 혜택") + " 합침";
        await api("update", { table: "이벤트", no: target.no, fields: packEventFields(next) });
        const r2 = await uploadShots(target.no, imgs, (i, n) => { b.innerHTML = "<b>캡처 저장 " + i + "/" + n + "</b>"; });
        closeModal();
        toast(r2.failed ? "합쳤어요. 캡처 " + r2.failed + "장은 저장하지 못했습니다." : "혜택 " + bens.length + "건을 합쳤어요.", !!r2.failed);
        await loadAll();
        S.tab = "promo"; render();
      } catch (e) {
  ...
  if (over) over.onclick = async () => {
    ...
    try {
      ...
      next.memo = dup.memo;
      await api("update", { table: "이벤트", no: dup.no, fields: packEventFields(next) });
      const r2 = await uploadShots(dup.no, imgs, (i, n) => { over.textContent = "캡처 저장 " + i + "/" + n; });
      closeModal();
      toast(r2.failed ? "갱신했어요. 캡처 " + r2.failed + "장은 저장하지 못했습니다." : "기존 이벤트를 갱신했어요.", !!r2.failed);
      await loadAll();
      S.tab = "promo"; render();
    } catch (e) {
      toast(e.message, true);
  ...
  out.querySelector("#an-add").onclick = async () => {
    ...
    try {
      const res = await api("add", { table: "이벤트", fields: packEventFields(buildFields()) });
      const r2 = await uploadShots(res.no, imgs, (i, n) => { btn.textContent = "캡처 저장 " + i + "/" + n; });
      closeModal();
      toast(r2.failed ? "담았어요. 캡처 " + r2.failed + "장은 저장하지 못했습니다." : "후보군에 담았어요.", !!r2.failed);
      await loadAll();
      S.tab = "promo"; render();
    } catch (e) {
      toast(e.message, true);
...
function renderReport(main) {
  const R = crossReport(S.user.name);
...
function renderStats(main) {
  if (!S.statsLoaded) {
    main.appendChild(el(`<div class="loading">불러오는 중...</div>`));
    loadStats().then(() => { S.statsLoaded = true; if (S.tab === "stats") render(); })
      .catch(e => {
        S.statsLoaded = true;
        if (S.tab === "stats") {
          main.innerHTML = "";
          main.appendChild(el(`<div class="empty">불러오지 못했어요.<br>${esc(e.message)}</div>`));
        }
      });
    return;
  }
  ...
  const st = S.stats;
  if (!st) {
    main.appendChild(el(`<div class="empty">지금은 집계를 제공하지 않고 있어요.</div>`));
    return;
  }

  main.appendChild(el(`
    <section class="hero stats">
      <div class="hero-label">지금 사람들이 하고 있는 것</div>
      <div class="hero-num">${st.users}<span class="won">명</span></div>
      <div class="hero-sub">참여 중이거나 후보로 담아둔 사람 수</div>
    </section>`));

  main.appendChild(el(`<div class="note small">누가 무엇을 하는지는 보이지 않습니다. 인원수만 셉니다. ${st.min}명 미만인 항목은 특정될 수 있어 아예 빼고 집계합니다.</div>`));

  /* 카드사 순위 */
  if (st.issuers.length) {
    const max = st.issuers[0].count || 1;
    main.appendChild(el(`<div class="sec-head"><h2>많이 신청한 카드사</h2></div>`));
    main.appendChild(el(`
      <section class="rank">
        ${st.issuers.map(r => {
      const col = CONFIG.ISSUER_COLORS[r.name] || ["#333", "#666"];
      const j = hasMyData() ? judge(r.name, issuerCondInfo(r.name).cond) : null;
      return `<div class="rk">
              <div class="rk-top">
                <b>${esc(r.name)}</b>
                ${j ? (j.hardBlock ? '<span class="tag">대상 제외</span>'
        : j.ok ? '<span class="tag live">나도 가능</span>'
          : `<span class="tag">D-${j.wait}</span>`) : ""}
                <span class="rk-amt">${r.count}명</span>
              </div>
              <div class="rk-bar"><i style="width:${Math.round(r.count / max * 100)}%;background:linear-gradient(90deg,${col[0]},${col[1]})"></i></div>
            </div>`;
    }).join("")}
      </section>`));
  }
  ...
  if (st.platforms.length) {
    const max = st.platforms[0].count || 1;
    main.appendChild(el(`<div class="sec-head"><h2>많이 쓰는 캐시백 플랫폼</h2></div>`));
    main.appendChild(el(`
      <section class="rank">
        ${st.platforms.map(r => `
          <div class="rk">
            <div class="rk-top"><b>${esc(r.name)}</b><span class="rk-amt">${r.count}명</span></div>
            <div class="rk-bar"><i style="width:${Math.round(r.count / max * 100)}%;background:linear-gradient(90deg,#3D6BFF,#7DA0FF)"></i></div>
          </div>`).join("")}
      </section>`));
  }
  ...
  /* 프로모션 */
  if (st.promos.length) {
    main.appendChild(el(`<div class="sec-head"><h2>사람들이 담아둔 프로모션</h2><span class="sec-sub">응모 가능한 것만</span></div>`));
    st.promos.forEach(p => {
      const dd = p.endApply ? daysTo(p.endApply) : null;
      let badge = "";
      if (hasMyData()) {
        const mine = S.events.some(e => e.issuer === p.issuer && (e.status === "신청" || e.status === "후보"));
        const j = judge(p.issuer, p.cond);
        badge = mine ? '<span class="pill done">나도 참여 중</span>'
          : j.hardBlock ? '<span class="pill no">대상 제외</span>'
            : j.ok ? '<span class="pill ok">나도 가능</span>'
              : `<span class="pill wait">D-${j.wait}</span>`;
      }
      const row = el(`
        <section class="fr tapcard" tabindex="0" role="button">
          <div class="fr-head">
            <div>
              <div class="promo-issuer">${esc(p.issuer)}${p.platform ? " | " + esc(p.platform) : ""}</div>
          ...
          <div class="fr-sub">
            <b>${p.count}명이 담음</b>
            ${p.total ? `<b class="green">최대 ${fWon(p.total)}</b>` : ""}
            ${p.benefits ? `<span>혜택 ${p.benefits}건</span>` : ""}
            ${dd != null && dd >= 0 ? `<span>응모 마감 ${fDateK(p.endApply)} (D-${dd})</span>` : ""}
          </div>
        </section>`);
      row.onclick = () => openPromoStat(p);
      row.onkeydown = e => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); openPromoStat(p); } };
      main.appendChild(row);
    });
  }

  if (!st.issuers.length && !st.platforms.length && !st.promos.length) {
    main.appendChild(el(`<div class="empty">아직 보여드릴 만큼 쌓이지 않았어요.<br>${st.min}명 이상이 담은 항목부터 나옵니다.</div>`));
  }
}

function openPromoStat(p) {
  const dd = p.endApply ? daysTo(p.endApply) : null;
  let mineBlock = "";
  if (hasMyData()) {
    const j = judge(p.issuer, p.cond);
    const already = S.events.some(e => e.issuer === p.issuer && (e.status === "신청" || e.status === "후보"));
    const checks = conditionChecks(p.issuer, p.cond);
    mineBlock = `
      <div class="fr-me ${already ? "done" : j.hardBlock ? "no" : j.ok ? "ok" : "wait"}">
        <b>${already ? "이미 같은 카드사로 참여 중이에요"
        : j.hardBlock ? "지금은 대상이 아니에요"
          : j.ok ? "지금 응모할 수 있어요" : `${fDateK(j.openDay)}부터 가능해요 (D-${j.wait})`}</b>
        ${checks.map(c => `
          <div class="ck ${c.ok ? "good" : "bad"}">
            <i>${c.ok ? "충족" : "미충족"}</i>
            <div class="ck-body">
              <div class="ck-rule"><b>${esc(c.name)}</b> ${esc(c.rule)}</div>
              <div class="ck-mine">내 상태 ${esc(c.mine)}${c.gap ? ", " + esc(c.gap) : ""}</div>
            </div>
          </div>`).join("")}
      </div>`;
  } else {
    mineBlock = `<div class="note">내 카드를 등록하면 내가 응모할 수 있는지도 알려드려요.</div>`;
  }
  ...
  openModal(`
    <div class="promo-head">
      <div>
        <div class="promo-issuer">${esc(p.issuer)}${p.platform ? " | " + esc(p.platform) : ""}</div>
        <h3 style="margin:2px 0 0">${esc(p.name)}</h3>
      </div>
    </div>
    <div class="fr-who">${p.count}명이 담아둔 프로모션${dd != null && dd >= 0 ? ", 응모 마감 " + fDateK(p.endApply) + " (D-" + dd + ")" : ""}</div>
    ${p.total ? `<div class="promo-total"><span>혜택 합계</span><b class="green">${fWon(p.total)}</b></div>` : ""}
    ${mineBlock}
    ${issuerTel(p.issuer) ? `<a class="callbtn" href="tel:${esc(issuerTel(p.issuer).replace(/-/g, ""))}">
      <b>${esc(p.issuer)}카드 ${esc(issuerTel(p.issuer))}</b>
      <span>내가 대상인지 물어보기</span>
    </a>` : ""}
    <p class="hint">여기 숫자는 이 앱을 쓰는 사람들의 집계일 뿐입니다. 자세한 조건은 카드사 공고를 직접 확인하고, 공고를 분석해 담아두면 내 조건에 맞춰 관리할 수 있습니다.</p>`);
}
...
function openSettings() {
  const mail = S.contact && S.contact.indexOf("@") > 0 ? S.contact : "";
  const m = openModal(`
    <h3>설정</h3>

    <div class="set-sec">
      <b>${esc(S.user.name)}님으로 로그인 중</b>
      <button class="mini ghost" id="st-logout">로그아웃</button>
    </div>

    <h4>문의하기</h4>
    <p class="hint">오류 제보나 건의를 남기면 확인 후 답을 드립니다. 답변받을 연락처를 함께 적어 주세요.</p>
    <label>분류
      <select id="st-kind">
        <option>오류 제보</option>
        <option>기능 건의</option>
        <option>개인정보 관련 요청</option>
        <option>기타</option>
      </select>
    </label>
    <label>내용<textarea id="st-body" rows="4" placeholder="어떤 화면에서 무엇을 하다가 생긴 일인지 적어 주시면 빠르게 확인할 수 있어요."></textarea></label>
    <button class="cta" id="st-send" style="margin-top:12px">보내기</button>
    ${mail ? `<p class="hint">메일로 보내셔도 됩니다. <a href="mailto:${esc(mail)}">${esc(mail)}</a></p>` : ""}
    ...
    <h4>탈퇴</h4>
    <p class="hint">탈퇴하면 등록한 카드, 프로모션, 캐시백 기록이 모두 지워집니다. 되돌릴 수 없습니다.</p>
    <button class="mini ghost danger" id="st-quit" style="margin-top:10px">탈퇴하고 내 데이터 모두 삭제</button>`);

  m.querySelector("#st-logout").onclick = () => { if (confirm("로그아웃할까요?")) logout(); };

  m.querySelector("#st-send").onclick = async () => {
    const btn = m.querySelector("#st-send");
    const body = m.querySelector("#st-body").value.trim();
    if (body.length < 5) { toast("내용을 조금 더 자세히 적어 주세요.", true); return; }
    btn.disabled = true; btn.textContent = "보내는 중...";
    try {
      await api("contact", { kind: m.querySelector("#st-kind").value, body });
      closeModal();
      toast("문의를 접수했어요. 확인 후 답을 드리겠습니다.");
    } catch (e) {
      toast(e.message, true);
  ...
  m.querySelector("#st-quit").onclick = () => openQuit();
}

function openQuit() {
  const m = openModal(`
    <h3>정말 탈퇴할까요?</h3>
    <div class="promo-block bad">
      카드 ${S.cards.length}장, 프로모션 ${S.events.length}건이 모두 지워집니다.
      받은 캐시백 기록과 응모 이력도 함께 사라져 되돌릴 수 없습니다.
    </div>
    <p class="hint">잠시 쉬려는 것이라면 그냥 로그아웃만 하셔도 됩니다. 데이터는 그대로 남습니다.</p>
    <label>확인을 위해 이름을 그대로 적어 주세요<input id="qt-name" placeholder="${esc(S.user.name)}"></label>
    <div class="modal-btns wrapbtns">
      <button class="cta" id="qt-go" style="background:var(--red)">탈퇴하고 모두 삭제</button>
      <button class="mini ghost" id="qt-cancel">그만두기</button>
    </div>`);
  m.querySelector("#qt-cancel").onclick = () => closeModal();
  m.querySelector("#qt-go").onclick = async () => {
    const btn = m.querySelector("#qt-go");
    const v = m.querySelector("#qt-name").value.trim();
    if (v !== S.user.name) { toast("이름이 다릅니다.", true); return; }
    btn.disabled = true; btn.textContent = "삭제 중...";
    try {
      await api("deleteMe", { confirm: v });
      cacheClear();
      localStorage.removeItem(AUTH_KEY);
      alert("탈퇴가 완료되었습니다. 그동안 이용해 주셔서 고맙습니다.");
      location.reload();
    } catch (e) {
      toast(e.message, true);
      btn.disabled = false; btn.textContent = "탈퇴하고 모두 삭제";
    }
  };
}
