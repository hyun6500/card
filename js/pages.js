  ...
  btn.onclick = async () => {
    ...
    try {
      ...
      if (!r.candidates || !r.candidates.length) {
        out.innerHTML = '<div class="find-load">찾지 못했어요. 카드명을 정확히 적거나 직접 입력해 주세요.</div>';
      } else {
        out.innerHTML = `
          <div class="find-list">
      ...
      out.innerHTML = '<div class="find-load">' + esc(e.message) + "</div>";
    }
...
function promoCard(ev, isActive) {
  ...
  const c = el(`
    <section class="promo ${v.cls}">
      ...
      ${otherLine}
      <button class="promo-more">자세히 보기, 관리</button>
    </section>`);
...
function renderAnalysis(m, r, rawText, imgs) {
  ...
  out.querySelector("#an-add").onclick = async () => {
    ...
    try {
      await api("add", { table: "이벤트", fields: packEventFields(buildFields()), images: imgs || [] });
      closeModal();
      toast("후보군에 담았어요.");
      await loadAll();
      S.tab = "promo"; render();
