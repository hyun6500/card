...
function renderCards(main) {
  ...
  const head = el(`<div class="sec-head"><h2>쓰고 있는 카드</h2><span class="sec-sub">6개월 내 사용</span><button class="mini" id="add-card">+ 카드 추가</button></div>`);
  main.appendChild(head);
  head.querySelector("#add-card").onclick = () => openCardForm(null);

  if (!active.length) {
    main.appendChild(el(`<div class="empty">6개월 안에 쓴 카드가 없어요.<br>+ 카드 추가를 누르거나, 프로모션을 신청하면 카드가 자동으로 등록됩니다.</div>`));
  } else {
    main.appendChild(cardGrid(active));
  }

  if (resting.length) {
    main.appendChild(el(`<div class="sec-head"><h2>쉬고 있는 카드</h2><span class="sec-sub">6개월 넘게 미사용, 응모 조건에 유리</span></div>`));
    main.appendChild(cardGrid(resting));
  }
...
function cardGrid(cards) {
  ...
  cards.forEach(c => {
    ...
    const img = cardImg(c);
    const t = usageTier(c);
    const evs = S.events.filter(e => e.status === "신청" && e.issuer === c.issuer && e.owner === S.user.name);
    const qs = evs.map(quitSafeDay).filter(Boolean);
    const qsMax = qs.length ? maxDate(qs) : null;

    const box = el(`
      <div class="vc tapcard" style="--c1:${col[0]};--c2:${col[1]}" tabindex="0" role="button">
        <div class="vc-face${img ? " has-img" : ""}">
          ${img ? `<img src="${esc(img)}" alt="" referrerpolicy="no-referrer" onerror="this.closest('.vc-face').classList.remove('has-img');this.remove()">` : ""}
          <span class="vc-issuer">${esc(c.issuer)}</span>
          <span class="vc-chip"></span>
          <span class="vc-brand">${esc(c.brand || "")}</span>
        </div>
        <div class="vc-foot">
