...
function renderReport(main) {
  ...
  const data = yearReport(scope === "__all__" ? "" : scope);
  const totalPaid = data.reduce((s, y) => s + y.paid, 0);
  const totalExpect = data.reduce((s, y) => s + y.expect, 0);

  main.appendChild(el(`
    <section class="hero report">
      <div class="hero-label">지금까지 받은 캐시백</div>
      <div class="hero-num">${fWonFull(totalPaid)}<span class="won">원</span></div>
      <div class="hero-sub">${data.length ? data.length + "개 연도" : "기록 없음"}${totalExpect ? " | 받을 예정 " + fWonFull(totalExpect) + "원" : ""}</div>
    </section>`));
    ...
    const sec = el(`
      <section class="yr">
        <div class="yr-head">
          <b>${y.year}년</b>
          <span class="yr-sum">
            ${y.paid ? `<b>${fWonFull(y.paid)}원</b>` : ""}
            ${y.expect ? `<em class="soon">+${fWonFull(y.expect)}원 예정</em>` : ""}
          </span>
        </div>
        ${y.rows.map(r => `
          <div class="yr-row">
            <div class="yr-name">
              <b>${esc(r.issuer)}</b>
              ${r.status === "사용중" ? '<span class="tag live">사용 중</span>'
        : r.quit ? `<span class="tag">해지 ${fDateK(r.quit)}</span>`
          : r.status ? `<span class="tag">${esc(r.status)}</span>` : ""}
            </div>
            <div class="yr-amt">
              ${r.paid ? `<b>${fWonFull(r.paid)}</b>` : ""}
              ${r.expect ? `<em class="soon">${r.paid ? "+" : ""}${fWonFull(r.expect)} 예정</em>` : ""}
            </div>
          </div>`).join("")}
      </section>`);
  ...
  main.appendChild(el(`<p class="hint" style="text-align:center">DB 스프레드시트에 "결산" 시트가 만들어집니다. 누를 때마다 최신 내용으로 다시 씁니다.</p>`));
}
