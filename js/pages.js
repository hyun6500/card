...
function render() {
  ...
  else if (S.tab === "friends") renderFriends(main);
      ...
      const note = el(`
        <section class="chance">
          <div class="chance-body tapcard" tabindex="0" role="button">
            <div class="chance-tag">지금 응모할 수 있어요${ch.kind === "friend" ? " | " + esc(ch.fr.owner) + "님이 참여 중" : ""}</div>
...
function renderFriends(main) {
  main.appendChild(el(`<div class="sec-head"><h2>다른 사람들이 하는 것</h2><span class="sec-sub">참여 중인 프로모션</span></div>`));

  if (!S.friendsLoaded) {
    main.appendChild(el(`<div class="loading">불러오는 중...</div>`));
    loadFriends().then(() => { S.friendsLoaded = true; if (S.tab === "friends") render(); })
      .catch(e => {
  ...
  if (!S.friends.length) {
    main.appendChild(el(`<div class="empty">아직 다른 사람이 참여 중인 프로모션이 없어요.</div>`));
    return;
  }

  if (!hasMyData()) {
    main.appendChild(el(`<div class="note">내 카드를 등록하면 각 프로모션에 내가 응모할 수 있는지도 함께 보여드려요.</div>`));
  }

  const byOwner = {};
  S.friends.forEach(f => { (byOwner[f.owner] = byOwner[f.owner] || []).push(f); });

  Object.keys(byOwner).sort().forEach(owner => {
    const list = byOwner[owner];
    main.appendChild(el(`<div class="owner-head"><b>${esc(owner)}</b><span>${list.length}건 참여 중</span></div>`));
    list.forEach(f => {
      const total = (f.benefits || []).reduce((s, b) => s + Number(b.back || 0), 0);
      const dd = f.endApply ? daysTo(f.endApply) : null;
      let badge = "";
      if (hasMyData()) {
        const j = judge(f.issuer, f.cond);
        const mine = S.events.some(e => e.issuer === f.issuer && (e.status === "신청" || e.status === "후보"));
      ...
      const row = el(`
        <section class="fr tapcard" tabindex="0" role="button">
          <div class="fr-head">
            <div>
              <div class="promo-issuer">${esc(f.issuer)}${f.platform ? " | " + esc(f.platform) : ""}${f.brand ? " | " + esc(f.brand) : ""}</div>
              <div class="promo-name">${esc(f.name)}</div>
            </div>
            ${badge}
          </div>
          <div class="fr-sub">
            ${total ? `<b class="green">최대 ${fWon(total)}</b>` : ""}
            ${(f.benefits || []).length ? `<span>혜택 ${f.benefits.length}건</span>` : ""}
            ${dd != null && dd >= 0 ? `<span>응모 마감 ${fDateK(f.endApply)} (D-${dd})</span>`
          : f.endApply ? `<span class="off-txt">응모 마감됨</span>` : ""}
          </div>
        </section>`);
      row.onclick = () => openFriendDetail(f);
      row.onkeydown = e => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); openFriendDetail(f); } };
      main.appendChild(row);
    });
  });
}
...
function openFriendDetail(f) {
  ...
  if (hasMyData()) {
    const j = judge(f.issuer, f.cond);
    const already = S.events.some(e => e.issuer === f.issuer && (e.status === "신청" || e.status === "후보"));
    const checks = conditionChecks(f.issuer, f.cond);
    mineBlock = `
  ...
  openModal(`
    <div class="promo-head">
      <div>
        <div class="promo-issuer">${esc(f.issuer)}${f.platform ? " | " + esc(f.platform) : ""}${f.brand ? " | " + esc(f.brand) : ""}</div>
        <h3 style="margin:2px 0 0">${esc(f.name)}</h3>
      </div>
    </div>
    <div class="fr-who">${esc(f.owner)}님이 참여 중${f.applied ? ", " + fDateK(f.applied) + " 신청" : ""}</div>
    ${dd != null && dd >= 0 ? `<div class="promo-apply">응모 마감 ${fDateK(f.endApply)} (D-${dd})</div>`
      : f.endApply ? `<div class="promo-apply">응모 마감됨 (${fDateK(f.endApply)})</div>` : ""}
    ${f.summary ? `<p class="an-sum">${esc(f.summary)}</p>` : ""}
    ${mineBlock}
    <div class="promo-total"><span>혜택 합계</span><b class="green">${fWon(total)}</b></div>
    ${(f.benefits || []).map(b => `
      <div class="ben">
        ...
        <div class="ben-date">${b.need ? fWon(b.need) + " 결제 | " : ""}${b.payS ? fDateK(b.payS) + "~" : ""}${b.payE ? fDateK(b.payE) : ""}${b.when ? " | 지급 " + esc(b.when) : ""}</div>
        ${b.keep ? `<div class="ben-keep">유지: ${esc(b.keep)}</div>` : ""}
        ${(b.todos || []).length ? `<div class="ben-todo">확인할 것: ${esc(b.todos.join(", "))}</div>` : ""}
      </div>`).join("")}
    ${(f.cautions || []).length ? `<h4>꼭 알아야 할 것</h4><ul class="cautions">${f.cautions.map(t => `<li>${esc(t)}</li>`).join("")}</ul>` : ""}
    ${issuerTel(f.issuer) ? `<a class="callbtn" href="tel:${esc(issuerTel(f.issuer).replace(/-/g, ""))}">
      <b>${esc(f.issuer)}카드 ${esc(issuerTel(f.issuer))}</b>
      <span>내가 대상인지 물어보기</span>
    </a>` : ""}
    <p class="hint">공고를 직접 확인한 뒤 응모하세요. 조건은 사람마다 다르게 적용될 수 있습니다.</p>`);
}
