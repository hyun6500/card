...
function applyRows(out) {
  ...
  // 서버가 본인 것만 보내지만, 예전 배포와 섞여도 안전하도록 한 번 더 거른다
...
async function loadAll() {
  applyRows(await api("rows"));
}

/* 마지막으로 받아온 데이터를 이 기기에 남겨둔다.
   다음에 열 때 서버를 기다리지 않고 곧바로 화면을 그리기 위해서다. */
const CACHE_KEY = "cardapp_cache";

function cacheSave(out) {
  try {
    localStorage.setItem(CACHE_KEY, JSON.stringify({
      who: S.user && S.user.name, at: Date.now(),
      cards: out.cards || [], events: out.events || []
    }));
  } catch (e) { }
}
...
function cacheLoad() {
  try {
    ...
    if (!c || !S.user || c.who !== S.user.name) return false;
    S.cards = (c.cards || []).map(normCard);
    S.events = (c.events || []).map(normEvent);
    return S.cards.length > 0 || S.events.length > 0;
...
async function loadFriends() {
  ...
  S.friends = (out.friends || []).map(f => {
    f.benefits = (f.benefits || []).map(b => Object.assign({}, b, {
      todos: (b.todos || []).map(t => (typeof t === "string" ? t : String(t.t || "")))
    }));
    f.cond = f.cond || {};
    return f;
  });
...
/* 내 카드 정보가 하나라도 있어야 자격을 따질 수 있다 */
function hasMyData() { return S.cards.length > 0; }

/* ================================================================
   홈 알림: 지금 응모할 수 있는 것 중 마감이 가장 가까운 하나
   ================================================================ */
const SNOOZE_KEY = "cardapp_snooze";
...
/* 후보군과 친구들 프로모션 중 지금 응모 가능한 것들을 마감 가까운 순으로 */
function openChances() {
  const out = [];
  const seen = {};

  S.events.filter(e => e.status === "후보").forEach(e => {
    const v = verdictOf(e);
    if (v.cls !== "ok") return;
    const end = pDate(e.endApply);
    if (end && end < today()) return;
    const total = (e.data.benefits || []).reduce((s, b) => s + Number(b.back || 0), 0);
    seen[e.issuer + "|" + e.name] = 1;
    out.push({ kind: "mine", ev: e, issuer: e.issuer, name: e.name, end, total, dd: end ? daysTo(end) : null });
  });

  (S.friends || []).forEach(f => {
    if (seen[f.issuer + "|" + f.name]) return;
    // 같은 카드사로 내가 이미 참여 중이면 권할 일이 아니다
    if (S.events.some(e => e.issuer === f.issuer && (e.status === "신청" || e.status === "후보"))) return;
    const j = judge(f.issuer, f.cond);
    if (!j.ok) return;
    const end = pDate(f.endApply);
    if (end && end < today()) return;
    const total = (f.benefits || []).reduce((s, b) => s + Number(b.back || 0), 0);
    out.push({ kind: "friend", fr: f, issuer: f.issuer, name: f.name, end, total, dd: end ? daysTo(end) : null });
  });
...
/* 카드사 고객센터 번호 */
function issuerTel(issuer) { return (CONFIG.ISSUER_TEL || {})[issuer] || ""; }
