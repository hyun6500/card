...
function openChances() {
  ...
  ((S.stats && S.stats.promos) || []).forEach(p => {
    if (seen[p.issuer + "|" + p.name]) return;
    // 같은 카드사로 내가 이미 참여 중이면 권할 일이 아니다
    if (S.events.some(e => e.issuer === p.issuer && (e.status === "신청" || e.status === "후보"))) return;
    const j = judge(p.issuer, p.cond);
    if (!j.ok) return;
    const end = pDate(p.endApply);
    if (end && end < today()) return;
    out.push({ kind: "popular", pr: p, issuer: p.issuer, name: p.name, end,
      total: Number(p.total || 0), dd: end ? daysTo(end) : null, count: p.count });
  });
