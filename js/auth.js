...
function showLogin() {
  ...
  async function enter(id, pw, name) {
    ...
    if (out.email) S.contact = out.email;
  ...
  async function go() {
    ...
    try {
      if (mode === "login") {
        ...
        if (!agree) throw new Error("이용약관과 개인정보처리방침에 동의해야 가입할 수 있어요.");
        await api("signup", { newId: id, newPw: pw, newName: name, invite, agree: true });
